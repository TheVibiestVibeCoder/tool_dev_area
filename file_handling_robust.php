...
/**
 * ATOMIC UPDATE - General purpose
 * Für Admin-Funktionen die gesamte Daten-Array modifizieren
 */
 ...
 */
function saveConfig($configFile, $config, $maxRetries = 10, $retryDelay = 100000) {
    $attempts = 0;
    ...
    while ($attempts < $maxRetries) {
        ...
        if (flock($fp, LOCK_EX)) {
            ...
            if ($json === false) {
                logError("JSON encode error for config: " . json_last_error_msg());
                flock($fp, LOCK_UN);
                fclose($fp);
                return false;
            }
            ...
            if ($writeResult !== false) {
                // ✅ AUTO-BACKUP ERSTELLEN
                createAutoBackup($configFile);
                return true;
            } else {
                logError("Failed to write config on attempt $attempts");
                if ($attempts < $maxRetries) {
                    usleep($retryDelay);
                    continue;
                }
            ...
            logError("Failed to acquire lock for config save");
            return false;
        }
    }
